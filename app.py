...
if view_mode == "HITSCAN/HITPLAN/HITSERIES（→ モニタリング）":
    ...
    **HITSCAN/HITPLAN/HITSERIES（→ モニタリング）**
    ...
    ３つの球と白い球が最初にある。白い球と３つの球は最初は接続なしから始まり、順次:

    1. **HITSCAN** — 白い球が一つずつ、赤・青・緑の順に接続。赤→青→緑の順でパルスが増えていく。
    2. **HITPLAN** — 白い球と接続された３つ巴の回転によるマッピング。神経接続が安定するにつれ、軌道円の不透明度を徐々に上げ、回転によるマッピングが描かれる。
    3. **HITSERIES CICD** — 神経パルスによる形状観測・継続学習すると、空間のトポロジーが推測でき学習が済むと等高線が現れる。３つの球が回転することで空間が摩耗し、空間の下部に穴が発生することが見えてくる。
...
if view_mode == "HITSCAN/HITPLAN/HITSERIES（→ モニタリング）":
    ...
    st.sidebar.markdown("- 💛 **下部の穴** — 回転で空間が摩耗し発生して見えてくる")
